// $Id: Cluster.cpp,v 1.44 2008/09/16 01:44:47 samn Exp $ 
...
{	extern int iSQRTCalls; iSQRTCalls = 0;
	try
	{
		...
		const int iBestDims = Clusters.m_oCQO.m_iBestDims;
...
prob_t Entropy(myhash& H,double sz,double maxbin)
{
	...
	for(;IT!=H.end();IT++)
	{
		...
		for(;JT!=H.end();JT++)
		{
			if(JT==IT || JT->second<1) continue;
			int dist = 0; int f = 0;
			for(s=0;s<dims;s++){
				f = abs(IT->first[s]-JT->first[s]);
				f*=f;
				dist += f;
			}
			//dist = sqrt((double)dist);
			double w = pow(2.71828183,-dist);
			p.push_back(JT->second*w);
			//p.push_back(JT->second/(double)dist);
		}
		double d = Sum(p) / sz;
		dEntrop += d * log2(d);
	}
