// $Id: WClustView.cpp,v 1.31 2008/09/16 01:45:32 samn Exp $ 
...
{
	CWClustDoc* pDoc = GetDocument();
	if(!this->m_bQBatchMode) ASSERT_VALID(pDoc);
	...
	if(m_bQBatchMode)m_oQB.bReadyToRun=false;

	int ErrorNumb=oS.LoadVerx(&errChar,&errOffset,m_bQBatchMode?&m_oQB:0);
	m_Redraw = 1;
...
//			pDoc->m_MainClusters.Clustering(&pDoc->m_MainDataStack);
	...
	if(m_bQBatchMode)m_oQB.bReadyToRun=true;
	return true;
}
...
{
	CWClustDoc* pDoc = GetDocument();
	if(!m_bQBatchMode) ASSERT_VALID(pDoc);
	...
	CString strInfo = GetClusterInfoString(pDoc->m_MainDataStack,pDoc->m_MainClusters,GetSafeHwnd());
...
void CWClustView::OnFileLoadclusters() 
{
	...
	CFile file;
	int errorNumb = oC.OpenFile(&file,&oS,m_bQBatchMode?&m_oQB:0);
...
void CWClustView::OnFileStoretobpfile() 
{
	...
	if(m_bQBatchMode)
		oS.whichDraw = CLUST_ORIG;
	else if(oS.whichDraw != CLUST_USER)
	{	CString str;
	...
	{	// store BPF
		...
		if (error == 0)
		{
			while (flag == 5)
			{
				flag=oS.StoreBPF(&State,m_bQBatchMode);
				//,m_bQBatchMode?(oS.m_SpikesStack.begin()+oS.GetForStoreBPFSpikeIndex())->TimeStamp:-1);
...
void CWClustView::OnToolsExport() 
{
	...
	if ( fileTXT != NULL )
	{
		...
		//get info gain for each cluster as string to be written to header
		//pDoc->m_MainDataStack.whichDraw = CLUST_USER;
		CString strInfo = GetClusterInfoString(oS,pDoc->m_MainClusters);
...
{
	char pomT[80];
	//sprintf(pomT,"Can't perform KlustaKwik clustering!");
	//CWnd::MessageBox(pomT,"WClust",MB_ICONERROR);
	//return;
...
//		/**pEntropy =*/ pDoc->m_MainClusters.EntropyInfoClustMD(pDoc->m_MainDataStack,iClusts,iIters,pCUPDUPData);
...
bool TSBThread(const CUPDUPDATA* pUp)
{
	...
	for(iB=0;iB<iSz && !pUp->ShouldTerminate();iB++)
	{
		...
		{ 
			...
			sprintf(msg,"Running TimeStamp Adjust Batch Job %d of %d : T%d",iB+1,iSz,oB.iTetrode+1);
...
void CWClustView::OnToolsExportspikes()
{
	...
	if(!oS.ExportSpikes(strFile.GetBuffer()))
	{
		CString strTmp;
